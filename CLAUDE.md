...
- **Reflection-free invocation (optional)**: `langchain4j-cdi-mcp-server` defines a container-agnostic invocation SPI (`McpMethodInvoker`/`McpInvokerProvider`, CDI 4.0.1, no `jakarta.enterprise.invoke` reference) that `McpBeanInvoker` consults before falling back to `Method.invoke`. The optional `langchain4j-cdi-mcp-invoker-cdi41` module implements it with the CDI 4.1 invoker API (verified on Quarkus/ArC only) — **CDI 4.1 is a hard prerequisite**: adding it to a CDI 4.0 host (e.g. Helidon, which ships Weld 5.1.6) breaks the deployment rather than degrading gracefully.
